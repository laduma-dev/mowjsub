...
class TestPyBDSFAsciiCatalogues(unittest.TestCase):
    """The formats a pipeline's source finder actually emits."""

    def test_a_pybdsf_srl_is_read_by_its_comment_header(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "x.srl"
            path.write_text(
                "# Source list for foo.fits\n"
                "# Generated by PyBDSF version 1.14.1\n"
                "# Source_id Isl_id RA E_RA DEC E_DEC Total_flux Maj Min PA\n"
                "2 3 -112.183078 0.00008 -14.883771 0.0001 0.00117 0.0023 0.0020 163.9\n"
                "3 4 -112.188451 0.00006 -14.373084 0.0001 0.00137 0.0022 0.0018 173.1\n"
            )
            ra, dec, maj = read_catalogue(path)

        # Read by NAME, not position: column 0 is Source_id, not RA.
        np.testing.assert_allclose(ra, [360 - 112.183078, 360 - 112.188451], rtol=1e-9)
        np.testing.assert_allclose(dec, [-14.883771, -14.373084])
        np.testing.assert_allclose(maj, [0.0023, 0.0022])

    def test_a_sky_model_format_line_is_read(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "sky.txt"
            path.write_text("#format: Source_id RA DEC Total_flux DC_Maj DC_Min DC_PA\n2 -112.183078 -14.883771 0.00117 0.00093 0.0 62.7\n")
            ra, dec, maj = read_catalogue(path)

        np.testing.assert_allclose(ra, [360 - 112.183078], rtol=1e-9)
        np.testing.assert_allclose(dec, [-14.883771])
        np.testing.assert_allclose(maj, [0.00093])

    def test_negative_right_ascension_is_wrapped(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "cat.txt"
            path.write_text("-112.0 -14.0\n10.0 -14.0\n")
            ra, _, _ = read_catalogue(path)

        np.testing.assert_allclose(ra, [248.0, 10.0])

    def test_a_plain_list_is_still_positional(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "cat.txt"
            path.write_text("# ra dec\n150.0 -30.0\n")
            ra, dec, _ = read_catalogue(path)

        np.testing.assert_allclose(ra, [150.0])
        np.testing.assert_allclose(dec, [-30.0])

    def test_a_srl_with_a_trailing_string_column_is_read(self):
        """PyBDSF ends its source list with S_Code, which is 'S' or 'M'."""
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "x.srl"
            path.write_text("# Source_id Isl_id RA DEC Total_flux Maj S_Code\n2 3 -112.183078 -14.883771 0.00117 0.0023 S\n3 4 -112.188451 -14.373084 0.00137 0.0022 M\n")
            ra, dec, maj = read_catalogue(path)
