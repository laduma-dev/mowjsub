...
Matching by name means an axis the WCS cannot type has nowhere to go, and mowjsub refuses rather than guessing -- a `NAXIS=4` cube with no `CTYPE4` (github issue #31) is malformed, not a layout to support. `unplaceable_axis_error` is the whole response: the repair is fitstoolz's, already, and the message prints both routes as commands you can paste (`fitstoolz remove-axis --ctype ''` to drop the axis, `fitstoolz header --add CTYPE4=STOKES` to name it). It reports the **FITS** axis number and CTYPE, not fitstoolz's dimension name -- `axis0`, what it used to say, appears in neither the file nor the repair command, and the ctype for an unset `CTYPE4` is the empty string, which no user would guess. `tests/test_main.py::TestZdsFromFits` pins the strings; they have been checked by pasting them.
